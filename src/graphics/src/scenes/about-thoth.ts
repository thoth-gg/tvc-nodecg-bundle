...
import { backgroundLogo, aboutThoth } from '../constant/image.ts';
import { DrawableEntitySet } from '../interfaces/entity.ts';
import { getDefaultBackground } from '../global/index.ts';

export class AboutThothScene  extends DrawableEntitySet {
  constructor(p5: p5js) {
    super(p5);
    ...
    this.entityList = [
      ...
      new AnimateImage(p5, aboutThoth, [
        new AnimateImageKey(backgroundLogo, { x: 0, y: 0, width: 1920, height: 1080, alpha: 0, sleep: 10 }),
        new AnimateImageKey(backgroundLogo, { x: 0, y: 0, width: 1920, height: 1080 }),
      ], {easing: 0.1}),
    ]
  }
