...
import { ReplaySceneInfo, SceneInfo } from "../../types/scene";
...
export async function initRoland(nodecg: NodeCG.ServerAPI) {
  ...
  const vr4hd = await Vr4hd.connect(portList.filter((port) => port != p20hd.port));
  ...
  nodecg.listenFor('overlay:replay:start', async () => {
    setTimeout(async () => {
      ...
      await sleep(1000)
      while (await p20hd.getPlayStatus().then(res => res.params[0] == "3").catch(() => true)) {
        await sleep(100)
      }
