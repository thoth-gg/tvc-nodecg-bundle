...
import { backgroundLogo, rules } from '../constant/image.ts';
import { DrawableEntitySet } from "../interfaces/entity.ts";
import { getDefaultBackground } from '../global/index.ts';

export class RulesScene extends DrawableEntitySet {
  constructor(p5: p5js) {
    super(p5);
    this.entityList = [
      ...
      new AnimateImage(p5, rules, [
        new AnimateImageKey(rules, { x: 0, y: 0, width: 1920, height: 1080, alpha: 0, sleep: 10 }),
        new AnimateImageKey(rules, { x: 0, y: 0, width: 1920, height: 1080 }),
      ], {}),
